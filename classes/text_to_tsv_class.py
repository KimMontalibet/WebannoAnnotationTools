...
import itertools
from WebannoAnnotationTools import taggers
import re
...
class text_to_tsv3:
    ...
    def add_tag(self, layer, variable, dict_regex, file = None,  
        ...
        self.token_dataframe[(layer, variable)] = self.token_dataframe.apply(lambda x: match_token_with_tag(x, list_tuples_span_tag, layer, variable, self.nb_desamb_id), axis = 1)
    ...
    def add_single_token_tag(self, layer, variable, dict_regex, token_lag = None): 
        if token_lag: 
            self.token_dataframe[("id_features", "token_lag")] = self.token_dataframe[("id_features", "token")].shift(periods=token_lag).fillna("")

        self.token_dataframe[(layer, variable)] = self.token_dataframe.apply(lambda x: match_single_token(x, dict_regex, token_lag, layer, variable), axis = 1)
...
def number_of_features(dict_features_by_type: Dict) -> int: 
    ...
    nb_features = sum([sum([len(x) for x in dict_features_by_type[key].values()]) for key in dict_features_by_type.keys()])   
    return nb_features 
...
def convert_to_webanno_tsv3(df: pd.DataFrame, text_par: List[str], 
    ...
    for i, group in dfg: 
        ...
        for row_index, row in group.iterrows():
            ...
            list_tuple_var = [[[tuple_var for tuple_var in list_tuple_var] for list_tuple_var in   dict_features_by_type[key].values()] for key in dict_features_by_type.keys()]
            list_tuple_var = list(itertools.chain.from_iterable(list(itertools.chain.from_iterable(list_tuple_var))))
            feature_row = [row[tuple_var] for tuple_var in list_tuple_var] + [""]
